...
def netstat():
	...
	date = time.strftime("%Y-%m-%d")
        path_to_file = "netstat_data_exit_ten_"+date+".json"
        with open(path_to_file,"a+") as f:
            f.write(str(result))
            f.write(str("\n"))

        infile = "netstat_data_exit_ten_"+date+".json"
        outfile = "netstat_data_exit_ten_formatted_"+date+".json"
	...
	#date = time.strftime("%Y-%m-%d")
        ...
        path_to_file = "netstat_data_exit_ten_IP_"+date+".json"
        with open(path_to_file,"a+") as f:
            f.write(str(result))
            f.write(str("\n"))

        infile = "netstat_data_exit_ten_IP_"+date+".json"
        outfile = "netstat_data_exit_ten_IP_formatted_"+date+".json"
        ...
        #Removes duplication
        lines = open('netstat_data_exit_ten_IP_formatted_'+date+'.json', 'r').readlines()
        lines_set = set(lines)
        out  = open('netstat_data_exit_ten_IP_formatted_'+date+'.json', 'w')
        ...
        with open('netstat_data_exit_ten_IP_formatted_'+date+'.json') as infile:
            for line in infile:
		    ...
		    #re.sub("'(^([0-9]{1,4}|[1-5][0-9]{4}|6[0-4][0-9]{3}|65[0-4][0-9]{2}|655[0-2][0-9]|6553[0-5])$+)'",'\\1',line)
                ...
                lines.append(line)
        with open('exit_ten_IP_data_'+date+'.json', 'w') as outfile:
            for line in lines:
                outfile.write(line)
